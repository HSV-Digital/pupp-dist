...
import { ResellerDownloadListDialog } from '@/components/dashboard/ResellerDownloadListDialog';
import {
	PdfDownloadTracker,
	type PdfDownloadJob,
} from '@/components/dashboard/PdfDownloadTracker';
import { PartnerCustomersCard } from '@/components/resellers/PartnerCustomersCard';
...
export default function ResellerDashboardPage() {
	...
	const { email, user, logout } = useResellerAuth();
	...
	const {
		...
		addCustomer,
		refresh,
	} = useResellerCustomers(apiFilters);
	...
	// PDF download tracker state
	const [downloadJobs, setDownloadJobs] = useState<PdfDownloadJob[]>([]);

	const handleAddDownloadJob = useCallback((job: PdfDownloadJob) => {
		setDownloadJobs((prev) => [...prev, job]);
	}, []);

	const handleRemoveDownloadJob = useCallback((jobId: string) => {
	...
	return (
		<div className="app-shell-content-wrap px-0!">
			<div className="flex h-80 w-full flex-col items-center justify-center rounded-b-xl bg-cover bg-position-[center_right]" style={{ backgroundImage: `url('${getThemeConfig().assets.dashboardHeroBanner}')` }}>
				<h1 className={`py-4 text-[42px] leading-[48px] font-bold ${getThemeConfig().styles.heroTextClass}`}>
					{t('resellerDashboard.heading')}
				</h1>
				<h4 className={`text-lg ${getThemeConfig().styles.heroTextClass}`}>
					{t('resellerDashboard.subheading')}
				</h4>
			</div>

			<div className="my-6 rounded-xl bg-cover bg-center p-6" style={{ backgroundImage: `url('${getThemeConfig().assets.dashboardBackground}')` }}>
				{renderContent()}
			</div>
			...
			<ResellerDownloadListDialog
				...
				sortDir={sortDir}
				onAddDownloadJob={handleAddDownloadJob}
				activeTrackerJobs={downloadJobs}
				onRemoveTrackerJob={handleRemoveDownloadJob}
			/>
			<PdfDownloadTracker
				jobs={downloadJobs}
				onRemoveJob={handleRemoveDownloadJob}
			/>
