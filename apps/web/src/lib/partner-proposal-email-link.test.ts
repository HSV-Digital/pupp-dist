...
import { createPartnerProposalEmailLink } from '@/lib/partner-proposal-email-link';
...
describe('createPartnerProposalEmailLink', () => {
	afterEach(() => {
		vi.clearAllMocks();
	});
	...
	it('returns parsed download URL payload on success', async () => {
		...
		const result = await createPartnerProposalEmailLink({
			journey: 'renewal',
			customerId: 'cust-1',
